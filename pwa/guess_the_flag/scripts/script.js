...
function addPoints(n){
  let pointTot = Number(localStorage.getItem("points"))
  let tot = pointTot + n
  localStorage.setItem("points", tot);
  console.log("tot = " ,tot)
}
...
function setupSelectionListeners() {
  ...
  const responsesElement = document.getElementById("reponses");
  ...
  responsesElement.addEventListener("click", e => {
    const block = e.target.closest(".block");
    if (!block) return;
    toggleSelection(block, selectedReponses);
    checkMatch();
  });
}

function toggleSelection(block, selectedArray) {
...
function checkMatch(){
  if (selectedItems.length === MAX_SELECTIONS && selectedReponses.length === MAX_SELECTIONS){
    const itemData = selectedItems[0].textContent.trim();
    const reponseIndex = selectedReponses[0].dataset.index;
    ...
    if (itemData === reponseCountry){
      ...
      selectedReponses[0].classList.remove("selected");
...
function createQuizBlocks(n){
  ...
  let randomNumberListe = [randomNumber];
  for (let i=1; i<n; i++){
    randomNumberListe.push((randomNumber + 500*i) % data.length);
  }

  const listeOfReponse = [];
  for (let i=0; i<n; i++){
    const a = getElementAtIndex(randomNumberListe[i]);
    const blockItems = document.createElement("div");
    const blockReponse = document.createElement("div");

    blockItems.classList.add("block");
    blockReponse.classList.add("block");

    blockItems.style.fontSize = "1.5em";
    blockItems.innerHTML = `<p>${a.country}</p>`;

    blockReponse.dataset.index = randomNumberListe[i];
    blockReponse.innerHTML = `<img src="${a.flag}" width="50">`;

    itemsElement.appendChild(blockItems);
    listeOfReponse.push(blockReponse);
  }

  shuffle(listeOfReponse).forEach(b => responsesElement.appendChild(b));
...
function initQuiz(gametype){
  switch (gametype){
    case "random":
      createQuizBlocks(range);
      break;

    default:
      console.log("Error: gametype invalide");
      return;
  }
